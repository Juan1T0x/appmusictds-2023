...
/**
 * Implementación de la interfaz CancionDAO utilizando JPA.
 * Proporciona métodos para agregar, actualizar, eliminar y consultar canciones en la base de datos.
 * 
 * @version 1.0
 */
public class JPACancionDAO implements CancionDAO {
    ...
    /**
     * Obtiene la instancia única de JPACancionDAO (patrón Singleton).
     * 
     * @return la instancia de JPACancionDAO.
     */
    ...
    @Override
    public int addCancion(String titulo, List<String> interpreteNombres, String estiloNombre, String url, int duracion) {
    ...
    @Override
    public void updateCancion(int id, String titulo, List<String> interpreteNombres, String estiloNombre, long numReproducciones) {
        EntityManager em = JPAUtil.getEntityManager();
        em.getTransaction().begin();
        ...
        if (cancion != null) {
            ...
            // Actualizar Interpretes
            List<Interprete> interpretes = interpreteNombres.stream().map(nombre -> findOrCreateInterprete(em, nombre)).collect(Collectors.toList());
            cancion.setInterpretes(interpretes);
    ...
    public void removeCancion(int id) {
        ...
        Cancion cancion = em.find(Cancion.class, id);
        if (cancion != null) {
            // Eliminar referencias de playlists
            List<Playlist> playlists = em.createQuery("SELECT p FROM Playlist p JOIN p.canciones c WHERE c.id = :cancionId", Playlist.class)
            ...
            // Eliminar referencias de canciones recientes de usuarios
            List<Usuario> usuarios = em.createQuery("SELECT u FROM Usuario u JOIN u.cancionesRecientes c WHERE c.id = :cancionId", Usuario.class)
                    .setParameter("cancionId", id).getResultList();
    ...
    public List<Cancion> getTopCanciones(int maxResults) {
        EntityManager em = JPAUtil.getEntityManager();
        TypedQuery<Cancion> query = em.createQuery("SELECT c FROM Cancion c ORDER BY c.numReproducciones DESC", Cancion.class);
    ...
    /**
     * Busca un estilo musical, en caso de no encontrar el estilo musical, se crea.
     * 
     * @param em entidad que maneja la base de datos.
     * @param nombre del estilo musical.
     * @return el estilo musical buscado o uno nuevo.
     */
    ...
    /**
     * Busca un intérprete, en caso de no encontrar el intérprete, se crea.
     * 
     * @param em entidad que maneja la base de datos.
     * @param nombre del intérprete.
     * @return el interprete buscado o uno nuevo.
     */
    private Interprete findOrCreateInterprete(EntityManager em, String nombre) {
        TypedQuery<Interprete> query = em.createQuery("SELECT i FROM Interprete i WHERE i.nombre = :nombre", Interprete.class);
        query.setParameter("nombre", nombre);
